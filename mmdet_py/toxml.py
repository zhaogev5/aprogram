...
import numpy as np
# xml_path='results/VOC2012/test/'
# 加载文件名称
# anno_result = pd.read_json(open("my_result1.json", "r"))
# file_name_list = list(set(anno_result['image_id']))
# print(file_name_list)
def to_xml(bboxes,xml_path,file_name):
...
#research部分
  ...
  pluginname_txt = doc.createTextNode('ship detection')
...
#object
  ...
  for box, score in zip(bboxes[:,:-1], bboxes[:,-1]):
      points = np.array(box)
      xmin = points[0]
      ymin = points[1]
      xmax = points[2]
